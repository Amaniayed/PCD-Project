...
export default function UploadData() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await recordsService.update(editId, form);
      } else {
        await recordsService.create(form);
      }
      setForm({ label: "", value: "" });
      setEditId(null);
      await fetchRecords();
  ...
  const handleEdit = (rec) => {
    setEditId(rec.id);
    setForm({ label: rec.label, value: rec.value });
  };
  ...
  return (
    <div className="upload-panel">
      ...
      <style>{`
        .upload-panel { color: #fff; font-family: 'DM Sans', sans-serif; }
        .upload-panel h2 { font-size: 20px; font-weight: 700; margin: 0 0 20px; color: #fff; letter-spacing: -0.3px; }
        .alert-error {
        ...
        .field label { font-size: 12px; font-weight: 500; color: rgba(255,255,255,0.5); text-transform: uppercase; letter-spacing: 0.5px; }
        .field input {
        ...
        tbody tr.editing { background: rgba(99,102,241,0.07); }
        td { padding: 12px 16px; font-size: 14px; color: rgba(255,255,255,0.75); }
        .id-cell { color: rgba(255,255,255,0.3); font-size: 12px; font-family: monospace; }
        .value-cell { font-family: monospace; color: #818cf8; }
        .date-cell { font-size: 12px; color: rgba(255,255,255,0.35); }
